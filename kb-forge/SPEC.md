...
(`~/superdev/docs/agentic-sdlc/`, chiefly docs 03/04/05/06) for a solo
...
(`<product>/superdev/`). From the import commit onward the product repo's git
...
`~/stocktickerapp/superdev/`.

## Design rules (non-negotiable)

1. **Tooling writes only inside `superdev/`.** Product code is never touched
...
2. **Read the product only at the pin** — the sha recorded in
   ...
   (`git log -1 -- . ':(exclude)superdev'`), so KB-only commits never move it.
...
Guards: commits touching only `superdev/` never re-trigger the loop (kills
...
(`git rev-parse --show-toplevel` → `<root>/superdev/`), so one kit serves
...
- `AGENTS.md` KB section (Codex + OpenCode read AGENTS.md natively;
  ...
  write lanes, pointer to KB-STATUS.md and the superdev-* procedures. Appended as
  one delimited section; developers own the rest of the file.
- `superdev/KB-STATUS.md`: agent-agnostic briefing (pin, sync, open queue
...
- Four procedures, single source `session-kit/skills/*/SKILL.md`:
  `superdev-recall` (ground the task; trust-class interpretation; no-coverage
  stop), `superdev-decide` (ADR-lite at decision time), `superdev-diverge` (file
  conflict, never self-ratify), `superdev-ratify` (founder ruling session; commits
  ...
  it natively), plus `.opencode/commands/` for explicit `/superdev-*` invocation
...
**Entry points:** `kbforge-init.sh <repo>` scaffolds superdev/ + seeds +

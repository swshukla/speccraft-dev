...
# superdev-ratify — the founder ruling session

1. Read `superdev/QUEUE.md` `## Open` (and any confrontation batches it
...
3. Apply each ruling to the right lane:
   ...
   - **Divergence ruled** → `superdev/ledger/DIV-NNN-<slug>.md` with ruling
   ...
   - **Any `fix-code` ruling (bug, divergence, contradiction, gotcha-violation)**
     → set its row in `superdev/findings/FINDINGS.md` to `confirmed` (or append
...
5. Commit: `KB_RATIFY=1 git commit -m "kb: rulings <date>" -- superdev`

---
description: Confine a fanned-out agent's edits to an assigned lane of the repo. Orchestrator sets the lane at launch; agents self-apply the boundary.
---
...
2. Under Claude Code, SessionStart materializes `SPECCRAFT_FREEZE` into a
   ...
   hard-denies any Edit/Write outside the lane. **OpenCode sessions don't
   get that hook** — there is no automatic enforcement here. Treat the
   lane as binding anyway and self-apply it: before every edit, check the
   target path against your assigned lane and stop if it's outside.
...
4. To widen a *running* session's lane (orchestrator decision), the
   ...
   Under OpenCode, the orchestrator can instead just restate the new lane
   in this command's `$ARGUMENTS` for a fresh invocation — pass the full

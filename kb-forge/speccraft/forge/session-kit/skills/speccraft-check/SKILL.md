...
under Claude Code, Codex, or OpenCode, or in CI, with no self-apply caveat.
It never mutates the product repo; it only reports.
...
1. **Run it.**
   ...
   `.speccraft/`). It prints a report grouped by check id and exits `0`
   (nothing strict-effective failed) or nonzero (≥1 strict-effective
   violation) — see mode 2 below for what "strict-effective" means.
